 ...
 */
class FlxDisabledButton extends FlxTypedButton<FlxBitmapText>
{
	/**
	...
	/**
	 * Creates a new `FlxDisabledButton` object with a gray background
	...
	inline function initLabel(Text:String):Void
	{
		if (Text != null)
		{
			...
			label.y = x + labelOffsets[status].y;
		}
	}
...
class FlxTypedButton<T:FlxSprite> extends FlxSprite implements IFlxInput
{
	...
	/**
	 * Shows the current state of the button, either `FlxDisabledButton.NORMAL`,
	 * `FlxDisabledButton.HIGHLIGHT` or `FlxDisabledButton.PRESSED`.
	 */
	 ...
	 */
	public var onUp(default, null):FlxDisabledButtonEvent;

	/**
	 * The properties of this button's `onDown` event (callback function, sound).
	 */
	public var onDown(default, null):FlxDisabledButtonEvent;

	/**
	 * The properties of this button's `onOver` event (callback function, sound).
	 */
	public var onOver(default, null):FlxDisabledButtonEvent;

	/**
	 * The properties of this button's `onOut` event (callback function, sound).
	 */
	public var onOut(default, null):FlxDisabledButtonEvent;
	...
	public function new(X:Float = 0, Y:Float = 0, ?OnClick:Void->Void)
	{
		...
		onUp = new FlxDisabledButtonEvent(OnClick);
		onDown = new FlxDisabledButtonEvent();
		onOver = new FlxDisabledButtonEvent();
		onOut = new FlxDisabledButtonEvent();

		status = FlxDisabledButton.NORMAL;
		...
		statusAnimations[FlxDisabledButton.HIGHLIGHT] = "normal";
		labelAlphas[FlxDisabledButton.HIGHLIGHT] = 1;
		#end
	...
	{
		loadGraphic("assets/images/button_disabled.png", false, 43, 15);
		// this.makeGraphic(43, 15, FlxColor.GRAY);
	}
	...
	function updateButton():Void
	{
		...
		if (status != FlxDisabledButton.NORMAL && (!overlapFound || (currentInput != null && currentInput.justReleased)))
		{
			onOutHandler();
		}
	}
	...
	function updateStatus(input:IFlxInput):Void
	{
		...
		else if (status == FlxDisabledButton.NORMAL)
		{
	...
	{
		if (visible && exists && active && status == FlxDisabledButton.PRESSED)
		{
			onUpHandler();
		}
	}
	...
	{
		status = FlxDisabledButton.PRESSED;
		input.press();
	...
	function onOverHandler():Void
	{
		...
		#end
		status = FlxDisabledButton.HIGHLIGHT;
...
/** 
 * Helper function for `FlxDisabledButton` which handles its events.
 */
private class FlxDisabledButtonEvent implements IFlxDestroyable
{
	/**
